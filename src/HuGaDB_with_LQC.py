...
def run_experiment(approach,n_trials,n_clients,hyper_params,noise_std,num_corrupted_clients,save_dir,sample_size,base_random_seed,data_root,subsample_size , corruption_settings):
    ...
    for hyper_param in hyper_params:
        ...
        for trial in range(n_trials):
            ...
            df_nash['Trial']=trial+1;df_nash['Noise Std']=noise_std
            df_nash['Corrupted Clients']=num_corrupted_clients;df_nash['Max Iter or Depth']=hyper_param\
            
            details_for_this_param.append(df_nash)

        all_details.append(pd.concat(details_for_this_param,ignore_index=True))
    ...
    final_details_df.to_csv(out_path,index=False);return final_details_df
