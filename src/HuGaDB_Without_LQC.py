...
def load_and_preprocess_global(train_pattern, test_pattern, subsample_test_size=None, test_random_state=42):
    ...
    # Optional subsample of the test set
...
AGG_MAP = {
    'fedavg': aggregate_lr_models,
    'majority_voting': FederatedForest
}
...
def run_basic (args):
    ...
    for trial in range(args.n_trials):
        ...
        for client_idx in range(args.n_clients):
            ...
            X_local = df_local.drop('act', axis=1)
            y_local = label_encoder.transform(df_local['act'])


            if args.model_type == 'logistic':
                model = LogisticRegression(random_state=args.random_seed + trial,max_iter=args.lr_max_iter)
                try:
                    local_scaler = StandardScaler()
                    model.fit(local_scaler.fit_transform(X_local), y_local)
                    client_models.append(model)
                    client_global_acc.append(model.score(X_test_global, y_test_global))
                except Exception:
                    client_models.append(None)
                    client_global_acc.append(None)
            else:
                model = DecisionTree(max_depth=args.lr_max_iter, random_state=args.random_seed + trial)
                model.fit(X_local, y_local)
                client_models.append(model)
                y_pred = model.predict(X_test_global)
                client_global_acc.append(accuracy_score(y_pred , y_test_global))

        df_res = evaluate_coalitions(
            client_models=client_models,
            client_global_accuracies=client_global_acc,
            n_clients=args.n_clients,
            aggregator_func=AGG_MAP[args.aggregator],
...
def run_evaluating(args):
    # Global preprocessing with subsample
    X_train_global, X_test_global, y_train_global, y_test_global, label_encoder, scaler = \
        load_and_preprocess_global(args.train_pattern, args.test_pattern, subsample_test_size=args.subsample_test_size, test_random_state=args.base_random_seed)
    ...
    for max_iter in args.max_iters:
        ...
        for trial in range(args.n_trials):
            ...
            trial_seed = args.base_random_seed + trial + int(1000 * max_iter) + 2 * rand_comp
            ...
            for client_idx in range(args.n_clients):
                df_local = pd.read_csv(args.train_pattern.format(i=client_idx+1)).dropna(subset=['act'])
                df_local, _ = train_test_split(df_local,train_size=args.sample_size,random_state=trial_seed,stratify=df_local['act'])
                ...
                if args.model_type == 'logistic':
                    ...
                    model = DecisionTree(max_depth= max_iter, random_state=trial_seed)
                    model.fit(X_local, y_local)
                    client_models.append(model)
            ...
            for idx in range(args.n_clients):
                acc_record[f'Client {idx+1} Accuracy'] = client_global_acc.get(idx, np.nan)
...
def parse_args():
    ...
    p_basic.add_argument('--train_pattern', type=str,default='/Users/abbaszal/Documents/Thesis_Project_Spambase/data/metadata/train_{i:02d}.csv')
    p_basic.add_argument('--test_pattern', type=str,default='/Users/abbaszal/Documents/Thesis_Project_Spambase/data/metadata/test_{i:02d}.csv')
    ...
    p_basic.add_argument('--lr_max_iter', type=int, default=100)
    p_basic.add_argument('--approach', type=str, default='fedlr')
    p_basic.add_argument('--aggregator',  type=str, choices=['fedavg','majority_voting'],default='fedavg')
    ...
    p_eval.add_argument('--train_pattern', type=str,default='/Users/abbaszal/Documents/Thesis_Project_Spambase/data/metadata/train_{i:02d}.csv')
    p_eval.add_argument('--test_pattern', type=str,default='/Users/abbaszal/Documents/Thesis_Project_Spambase/data/metadata/test_{i:02d}.csv')
    p_eval.add_argument('--subsample_test_size', type=int, default=950)
    p_eval.add_argument('--base_random_seed', type=int, default=42)
    ...
    p_eval.add_argument('--approach', type=str, default='fedlr')
    p_eval.add_argument('--aggregator',  type=str, choices=['fedavg','majority_voting'],default='fedavg')
...
if __name__ == '__main__':
    ...
    if args.command == 'basic':
        if args.approach == 'fedlr':
            args.save_dir = '/Users/abbaszal/Documents/Thesis_Project_Spambase/results/FedLR_HuGaDB_without_LQC'
            args.base_out = 'HuGaDB_results_with_LR.csv'
        else:  # fedfor
            args.save_dir =  '/Users/abbaszal/Documents/Thesis_Project_Spambase/results/FedFor_HuGaDB_without_LQC'
            args.base_out = 'HuGaDB_results_with_FedFor.csv'

    elif args.command == 'evaluating':
        if args.approach == 'fedlr':
            args.save_dir = '/Users/abbaszal/Documents/Thesis_Project_Spambase/results/FedLR_HuGaDB_without_LQC/evaluate_grand_combination_without_LQC'
        else:  # fedfor
            args.save_dir ='/Users/abbaszal/Documents/Thesis_Project_Spambase/results/FedFor_HuGaDB_without_LQC/evaluate_grand_combination_without_LQC'
