...
def basic_run(args, X_train, y_train, X_test, y_test):
    partitions = split_data_equal(X_train, y_train,n_clients=args.n_clients,shuffle=True,random_seed=args.seed)
    results = []
    for trial in range(args.n_trials):
        print(f"Trial {trial+1}/{args.n_trials}")
        models, accs = train_models(partitions, args.seed, X_test, y_test, args.max_iter)
...
def evaluating_run(args, X_train, y_train, X_test, y_test):
    ...
    for max_iter in args.max_iters:
        ...
        for trial in range(args.n_trials):
            ...
            trial_seed = args.seed + trial + int(1000*max_iter) + 2*seed_comp
...
def parse_args():
    ...
    p_basic.add_argument('--file_path',   type=str,   default='/Users/abbaszal/Documents/Thesis_Project_Spambase/data/spambase.data')
    p_basic.add_argument('--test_size',   type=float, default=0.2)
    p_basic.add_argument('--seed',        type=int,   default=42)
    ...
    p_eval.add_argument('--file_path',   type=str,   default='/Users/abbaszal/Documents/Thesis_Project_Spambase/data/spambase.data')
...
if __name__ == '__main__':
    ...
    X_train, X_test, y_train, y_test = load_and_preprocess_spambase(
        args.file_path, args.test_size, args.seed
    )

    if args.command == 'basic':
        if args.approach == 'fedlr':
            args.save_dir = '/Users/abbaszal/Documents/Thesis_Project_Spambase/results/FedLR_Spambase_without_LQC'
            args.base_out = 'Spambase_results_with_LR.csv'
        else:  # fedfor
            args.save_dir =  '/Users/abbaszal/Documents/Thesis_Project_Spambase/results/FedFor_Spambase_without_LQC'
            args.base_out = 'Spambase_results_with_FedFor.csv'

    elif args.command == 'evaluating':
        if args.approach == 'fedlr':
            args.save_dir = '/Users/abbaszal/Documents/Thesis_Project_Spambase/results/FedLR_Spambase_without_LQC/evaluate_grand_combination_without_LQC'
        else:  # fedfor
            args.save_dir ='/Users/abbaszal/Documents/Thesis_Project_Spambase/results/FedFor_Spambase_without_LQC/evaluate_grand_combination_without_LQC'
